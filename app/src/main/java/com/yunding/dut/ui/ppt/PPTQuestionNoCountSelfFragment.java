...
public class PPTQuestionNoCountSelfFragment extends BackHandledFragment implements IPPTContentView {
...
//    HorizontalListView horizontalListviewButton;
    ...
    protected int getLayoutId() {
        return R.layout.fragment_question_nocount_self_fragment;
...
//        mQuestionIndex = mPPTInfoItem.getSlideQuestions().indexOf(mPPTQuestionBean);
        ...
        mPPTInfo.getSlides().get(pptIndex - 1).setQuestionsCompleted(1);
            ...
            public void onClick(View v) {
                if (TextUtils.isEmpty(et_notes.getText().toString().trim())) {
                    ...
                    mPresenter.sendFeedBack(mPPTInfoItem.getTeachingId(), mPPTInfoItem.getSlideId()
                            , "", mPPTInfoItem.getClassId(), mPPTInfoItem.getSelfTaughtId() + "", et_notes.getText().toString().trim());
                    dialog.dismiss();
                }
            }
        });
